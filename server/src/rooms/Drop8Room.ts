...
type Drop8RoomMetadata={roomCode:string;hostName:string;players:number;humans:number;phase:'LOBBY'|'PLANE'|'DROP'|'ACTIVE'|'FINISHED';fillAi:boolean;publicRoom:boolean;mapSizeMode:MapSizeMode;mapDisplayName:string;createdAt:number;updatedAt:number;gameMode:GameMode;maxHumans:number;configuredAiCount:number;joinInProgress:boolean;lifecycle:'lobby'|'active'|'emptyGrace'|'disposed';killLimit:OpenArenaKillLimit;roundState:OpenArenaRoundState};
...
export class Drop8Room extends Room<{ state: Drop8State; metadata: Drop8RoomMetadata }> {
  ...
  async onCreate(options:JoinOptions) {
    ...
    this.openArenaConfig=this.gameMode==='openArena'?normalizeOpenArenaConfig(options.maxHumans,options.aiCount,options.killLimit):undefined;
    this.maxClients=this.openArenaConfig?.maxHumans??MAX_PLAYERS;
    if(this.gameMode==='openArena')this.autoDispose=false;
    this.state.fillAi=this.gameMode==='openArena'?(this.openArenaConfig?.configuredAiCount??0)>0:options.fillAi!==false;
  ...
  onJoin(client:Client,options:JoinOptions){
    ...
    if(this.gameMode==='openArena'&&this.state.phase==='ACTIVE'){
      if(this.openArenaRoundState==='active')this.resetOpenArenaCombatant(p,[...this.state.players.values()].filter((player)=>!player.ai).length-1,false);
      ...
      this.system(`${p.name}님이 진행 중인 상시 전장에 난입했습니다.`);
  ...
  async onLeave(client:Client,code:CloseCode){
    ...
    if(!reason&&this.state.phase!=='LOBBY'&&code!==CloseCode.CONSENTED){
      try{await this.allowReconnection(client,this.gameMode==='openArena'?10:15);return;}catch{/* reconnect timeout */}
    }
  ...
  private removePlayer(playerId:string,reason:'left'|'kicked'){
    ...
    else if(this.connectedHumanCount()===0)this.enterOpenArenaEmptyGrace();
    this.syncRoomRegistry();
  }
  ...
  private arenaStatusPayload(){
    ...
    return{lifecycle:this.openArenaLifecycle,humans,maxHumans:this.openArenaConfig?.maxHumans??MAX_PLAYERS,aliveAi,respawningAi:Math.max(0,(this.openArenaConfig?.configuredAiCount??0)-aliveAi),configuredAiCount:this.openArenaConfig?.configuredAiCount??0,totalCombatants:humans+(this.openArenaConfig?.configuredAiCount??0),hostId:this.state.hostId,killLimit:this.openArenaConfig?.killLimit??0,roundState:this.openArenaRoundState,roundEndsAt:this.openArenaRoundEndsAt,roundGeneration:this.openArenaRoundGeneration};
  }
  ...
  private enterOpenArenaEmptyGrace(){
    if(this.gameMode!=='openArena'||this.openArenaLifecycle!=='active')return;
  ...
  private updateOpenArenaLifecycle(){
    if(this.gameMode!=='openArena'||this.openArenaLifecycle!=='emptyGrace'||this.openArenaDisposeRequested)return;
    if(this.connectedHumanCount()>0){this.openArenaLifecycle='active';this.openArenaEmptyGraceUntil=0;this.broadcast('arenaStatus',this.arenaStatusPayload());this.syncRoomRegistry();return;}
  ...
  private beginOpenArena(){
    ...
    this.system(`상시 개방 전장이 시작되었습니다. 인간 ${this.openArenaConfig?.maxHumans??0}명 · AI ${this.openArenaConfig?.configuredAiCount??0}명 · ${this.openArenaConfig?.killLimit?`${this.openArenaConfig.killLimit}킬 제한`:'무제한'} · 자기장 없음`);
  }
  ...
  private input(c:Client,m:any){
    ...
    const scopeAllowed=p.phase==='landed'&&!p.werewolf.transformed&&!p.werewolf.transformPreparing&&!p.werewolf.ritualizing&&!p.isSwimming&&p.equipped==='sniper'&&!p.reloading&&!p.healingKind&&(!p.isDriving||(Math.hypot(input.x,input.y)<.05&&speedRatio<=MOTORCYCLE_SCOPE_SPEED_RATIO));
  ...
  private werewolfClaw(p:PlayerState){
    ...
    for(const vehicle of this.state.motorcycles.values()){if(vehicle.destroyed||distance(p.x,p.y,vehicle.x,vehicle.y)>WEREWOLF_BALANCE.clawRange+MOTORCYCLE_RADIUS)continue;if(Math.abs(this.angleDiff(Math.atan2(vehicle.y-p.y,vehicle.x-p.x),p.angle))>WEREWOLF_BALANCE.clawHalfAngleRadians)continue;if(this.firstObstacleHitT(p.x,p.y,vehicle.x,vehicle.y,2)!==null)continue;const second=vehicle.huntMarkedBy===p.id&&vehicle.huntMarkUntil>now;this.damageMotorcycle(vehicle,WEREWOLF_BALANCE.huntVehicleDamage,p.id,'늑대 할퀴기');if(vehicle.destroyed){vehicle.huntMarkedBy='';vehicle.huntMarkUntil=0;continue;}if(second&&vehicle.driverId){const driver=this.state.players.get(vehicle.driverId);if(driver&&now>=driver.werewolf.huntDismountImmuneUntil){this.forceDismountMotorcycle(driver,vehicle);driver.werewolf.actionLockedUntil=now+WEREWOLF_BALANCE.huntDriverLockSeconds;driver.werewolf.huntDismountImmuneUntil=now+WEREWOLF_BALANCE.huntDismountImmunitySeconds;w.attackRecoveryUntil=now+WEREWOLF_BALANCE.huntAttackRecoverySeconds;vehicle.huntMarkedBy='';vehicle.huntMarkUntil=0;continue;}}vehicle.huntMarkedBy=p.id;vehicle.huntMarkUntil=now+WEREWOLF_BALANCE.huntMarkSeconds;this.applyVehicleSlow(vehicle,'werewolf_hunt',`werewolf:${p.id}`,WEREWOLF_HUNT_VEHICLE_PROFILE);}
  ...
  private beginReload(p:PlayerState,weaponId:WeaponId=p.equipped as WeaponId){
    p.isSniperScoped=false;
    if(!p.alive||p.phase!=='landed'||p.isSwimming||p.isVaulting||p.werewolf.transformed||p.werewolf.transformPreparing||p.werewolf.ritualizing||this.now()<p.werewolf.actionLockedUntil||this.reloadUntil.has(p.id))return false;
  ...
  private mountMotorcycle(p:PlayerState,motorcycle:MotorcycleState){
    if(motorcycle.driverId||motorcycle.exploding||motorcycle.destroyed||p.isDriving||p.isSwimming||p.isVaulting||p.werewolf.transformed||p.werewolf.transformPreparing||p.werewolf.ritualizing||!p.alive||p.phase!=='landed')return false;
  ...
  private pickup(c:Client){
    const p=this.state.players.get(c.sessionId);
    if(!p||p.phase!=='landed'||!p.alive||p.isDriving||p.isSwimming||p.isVaulting||p.werewolf.transformed||p.werewolf.transformPreparing||p.werewolf.ritualizing)return;
  ...
  private swapWeaponSlots(c:Client,m:any){
    const p=this.state.players.get(c.sessionId);
    if(!p||!p.alive||p.phase!=='landed'||p.isVaulting||p.werewolf.transformed||p.werewolf.transformPreparing||p.werewolf.ritualizing||this.now()<p.werewolf.actionLockedUntil)return;
  ...
  private prepareThrow(c:Client){
    const p=this.state.players.get(c.sessionId);
    if(!p||p.ai||!p.alive||p.phase!=='landed'||p.isSwimming||p.isVaulting||p.werewolf.transformed||p.werewolf.transformPreparing||p.werewolf.ritualizing||this.now()<p.werewolf.actionLockedUntil||!isThrowableType(p.equipped)||p.throwableType!==p.equipped||p.throwableCount<=0)return;
  ...
  private beginHeal(p:PlayerState,requested:'auto'|HealKind='auto'){
    ...
    if(!p.alive||p.phase!=='landed'||p.isSwimming||p.isVaulting||p.werewolf.transformed||p.werewolf.transformPreparing||p.werewolf.ritualizing||this.now()<p.werewolf.actionLockedUntil||p.hp>=100||this.healUntil.has(p.id))return false;
  ...
  private tick(dt:number){
    ...
    if(this.gameMode==='openArena'&&this.openArenaLifecycle==='emptyGrace'){this.updateOpenArenaLifecycle();if(String(this.openArenaLifecycle)!=='active')return;}
    if(this.gameMode==='openArena'&&this.openArenaRoundState!=='active'){this.state.serverTime=this.now();this.updateOpenArenaRound();this.broadcastOpenArenaScoreboard();if(this.now()-this.registryLastSync>=2){this.registryLastSync=this.now();this.syncRoomRegistry();}return;}
    const tickStarted=performance.now();
    this.elapsed+=dt;
    this.state.serverTime=this.now();
    this.updatePlane(dt);
    this.updateWerewolfSeason(dt);
    ...
    if(this.gameMode==='openArena'){this.updateOpenArenaRespawns();this.updateOpenArenaWorld();this.broadcastOpenArenaScoreboard();}
  ...
  private updatePlayers(dt:number){
    for(const p of this.state.players.values()){
      ...
      if(p.ai)continue;
      if(p.werewolf.ritualizing||p.werewolf.transformPreparing||this.now()<p.werewolf.actionLockedUntil)continue;
  ...
  private updateBullets(dt:number){
    ...
    for(const [id,b] of this.state.bullets){
      ...
      if(hitPlayer){
        ...
        else this.damage(hitPlayer,b.damage,b.owner,'총기',0,undefined,'bullet');
  ...
  private moveAiWithAvoidance(p:PlayerState,intent:AiIntent,waypoint:AiRoutePoint,angle:number,speed:number,dt:number){
    // DROP8_AI_PATROL_STABILITY_AVOID
    const step=speed*dt;
    const sign=intent.avoidSign||1;
    const preferred=[0,sign*Math.PI/8,sign*Math.PI/4,sign*Math.PI*3/8,sign*Math.PI/2];
    const fallback=intent.stuckFor>.75?[-sign*Math.PI/8,-sign*Math.PI/4,-sign*Math.PI*3/8,-sign*Math.PI/2]:[];
    const reverse=intent.stuckFor>1.15?[Math.PI]:[];
    const offsets=[...preferred,...fallback,...reverse];
    ...
    for(const offset of offsets){
      ...
      const sidePenalty=Math.abs(offset)<Math.PI&&offset!==0&&Math.sign(offset)!==sign?42:0;
      const score=(before-after)*12-Math.abs(offset)*18-firePenalty-sidePenalty-(offset===Math.PI?110:0);
  ...
  private aiLootScore(p:PlayerState,k:LootKind){
    ...
    if(k==='adhesive_charge'||k==='silver_bolt'||k==='silver_crossbow')return 0;
  ...
  private damage(p:PlayerState,amount:number,attackerId:string,reason:string,knockbackOverride=0,hitAngleOverride?:number,kind:DamageKind='other'){
    ...
    if(p.ai&&attackerId&&attackerId!==p.id){this.aiDefendUntil.set(p.id,this.now()+3);const memory=this.ensureAiMemory(p);memory.lastThreatAt=this.now();const attackerPlayer=this.state.players.get(attackerId);if(attackerPlayer){memory.source='damage';memory.confidence=Math.max(memory.confidence,55);memory.heardX=attackerPlayer.x+(this.lootRandom()-.5)*120;memory.heardY=attackerPlayer.y+(this.lootRandom()-.5)*120;memory.heardAt=this.now();memory.heardKind='gun';}}
    ...
    if(attacker&&reason!=='자기장'){
      ...
      if(power>0){
        ...
        const effectivePower=p.werewolf.transformed&&resolvedKind==='explosion'?power*WEREWOLF_BALANCE.explosionKnockbackMultiplier:power;
    ...
    if(p.hp<=0){
      ...
      if(this.gameMode==='openArena'){if(this.openArenaRoundState==='active'){this.dropOpenArenaDeathLoot(p);this.scheduleOpenArenaRespawn(p);}this.broadcastOpenArenaScoreboard(true);}
      else this.dropInventory(p);
    }
  }

  private openArenaRoundLocked(){return this.gameMode==='openArena'&&this.openArenaRoundState!=='active';}
  ...
  private openArenaHumanRows():ArenaScoreRow[]{
    const rows=[...this.state.players.values()].map((player)=>{const deaths=this.openArenaDeaths.get(player.id)??0;return{id:player.id,name:player.name,ai:player.ai,kills:player.kills,deaths,kd:deaths>0?Number((player.kills/deaths).toFixed(2)):player.kills,streak:this.arenaKillStreak.get(player.id)??0,bestStreak:this.arenaBestStreak.get(player.id)??0,humanKills:this.arenaHumanKills.get(player.id)??0,aiKills:this.arenaAiKills.get(player.id)??0,damageDone:Number(player.damageDone??0),alive:player.alive,reachedAt:this.arenaKillReachedAt.get(player.id)??Number.MAX_SAFE_INTEGER};});
    return sortOpenArenaHumanRows(rows).slice(0,20);
  }
  ...
  private recordOpenArenaKill(attacker:PlayerState,victim:PlayerState){
    ...
    const row=this.openArenaHumanRows().find((candidate)=>candidate.id===attacker.id);if(row&&shouldFinishOpenArenaRound(this.openArenaConfig?.killLimit??0,row))this.finishOpenArenaRound(attacker,row);
  }

  private openArenaRoundPayload(){const winner=this.state.players.get(this.openArenaRoundWinnerId);return{roundState:this.openArenaRoundState,roundEndsAt:this.openArenaRoundEndsAt,resettingAt:this.openArenaRoundResettingAt,roundGeneration:this.openArenaRoundGeneration,killLimit:this.openArenaConfig?.killLimit??0,winnerId:this.openArenaRoundWinnerId,winnerName:winner?.name??'',rows:this.openArenaRoundRows};}
  ...
  private broadcastOpenArenaScoreboard(force=false){
    if(this.gameMode!=='openArena')return;const now=this.now();if(!force&&now<this.arenaScoreboardAt)return;this.arenaScoreboardAt=now+1;this.broadcast('arenaScoreboard',this.openArenaScoreboardPayload());
  }
  ...
  private syncRoomRegistry(){
    ...
    const metadata:Drop8RoomMetadata={
      ...
      maxHumans:this.openArenaConfig?.maxHumans??MAX_PLAYERS,
      configuredAiCount:this.openArenaConfig?.configuredAiCount??0,
      joinInProgress:this.gameMode==='openArena'&&this.state.phase==='ACTIVE'&&this.openArenaLifecycle==='active',
      lifecycle:this.gameMode==='openArena'?this.openArenaLifecycle:this.state.phase==='LOBBY'?'lobby':'active',
      killLimit:this.openArenaConfig?.killLimit??0,
      roundState:this.openArenaRoundState,
    };
    const arenaHidden=this.gameMode==='openArena'&&this.openArenaLifecycle!=='active';
  ...
  private getAmmo(p:PlayerState,t:AmmoType){return t==='pistol_ammo'?p.pistolAmmo:t==='standard_ammo'?p.standardAmmo:t==='shotgun_ammo'?p.shotgunAmmo:t==='rocket_ammo'?p.rocketAmmo:t==='fuel_ammo'?p.fuelAmmo:t==='adhesive_charge'?this.tacticalInventory(p.id).adhesiveCharge:t==='silver_bolt'?this.tacticalInventory(p.id).silverBoltAmmo:0;}
