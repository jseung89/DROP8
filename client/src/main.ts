...
function render(){
  ...
  $('aliveText').textContent=arena?`${Number(net.arenaStatus?.humans??s.players.filter((player:any)=>!player.ai).length)}인 + AI ${Number(net.arenaStatus?.aliveAi??s.players.filter((player:any)=>player.ai&&player.alive).length)}`:String(s.aliveCount);
