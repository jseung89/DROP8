...
describe('DROP 8 room integration', () => {
  ...
  const quiet = <T extends { onMessage: (type: string, callback: () => void) => unknown }>(client: T): T => {
    for (const type of ['chat', 'killfeed', 'result', 'error', 'notice', 'kicked', 'pickupResult', 'slotSwapResult', 'positionRecovery', 'vehicleRecovery', 'characterDeath', 'audioEvent']) {
  ...
  it('starts solo mode immediately with one human and diverse AI profiles', async () => {
    const room = await server.createRoom<Drop8State>('drop8', { soloMode: true, mapId: 'small' });
    const client = quiet(await server.connectTo(room, { nickname: 'SoloPlayer', soloMode: true }));
    const started = await waitForCondition(() => room.state.phase !== 'LOBBY' && room.state.players.size === 8, 2_500, 20);
    expect(started).toBe(true);
    expect(room.state.soloMode).toBe(true);
    expect([...room.state.players.values()].filter((player) => !player.ai)).toHaveLength(1);
    expect(new Set([...room.state.players.values()].filter((player) => player.ai).map((player) => player.aiProfile)).size).toBeGreaterThanOrEqual(3);
    expect(room.state.players.get(client.sessionId)?.name).toBe('SoloPlayer');
  });
  ...
  it('auto-equips a newly picked up weapon and publishes pickup feedback', async () => {
    ...
    try {
      ...
      const feedbackCall = sendSpy.mock.calls.find((call: unknown[]) => call[0] === 'pickupResult');
  ...
  it('publishes a non-blocking notice when healing is unavailable', async () => {
    ...
    expect(payload.message).toContain('붕대');
