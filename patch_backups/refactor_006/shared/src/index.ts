...
export const BUILDING_VISIBILITY_ZONES:BuildingVisibilityZone[]=BUILDINGS.map((building,index)=>{
...
export function buildingZoneById(id:string):BuildingVisibilityZone|undefined{return BUILDING_VISIBILITY_ZONES.find((zone)=>zone.id===id);}
export function buildingIdAt(x:number,y:number,inset=0):string{
  const zone=BUILDING_VISIBILITY_ZONES.find((item)=>x>=item.interior.x+inset&&x<=item.interior.x+item.interior.w-inset&&y>=item.interior.y+inset&&y<=item.interior.y+item.interior.h-inset);
  return zone?.id??'';
}
