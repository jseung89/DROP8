...
export type Snapshot={
  phase:string;roomCode:string;hostId:string;fillAi:boolean;publicRoom:boolean;difficulty:string;zoneSpeed:string;
...
export class Network {
  ...
  private attach(){
    ...
    for(const type of ['chat','killfeed','result','error','pong','kicked','pickupResult'])this.room.onMessage(type,(p:any)=>{
  ...
  private toSnapshot(s:any,receivedAt:number):Snapshot{
    const snapshot:Snapshot={
      phase:s.phase,roomCode:s.roomCode,hostId:s.hostId,fillAi:s.fillAi,publicRoom:s.publicRoom,difficulty:s.difficulty,zoneSpeed:s.zoneSpeed,
