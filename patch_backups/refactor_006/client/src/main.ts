...
async function connect(create:boolean){
  ...
  try{
    const opts={nickname:nick(),password:password.value,roomPassword:password.value,publicRoom:$<HTMLInputElement>('publicRoom').checked,fillAi:true,difficulty:'normal',zoneSpeed:'normal'};
...
type PublicRoomInfo={roomId:string;roomCode:string;hostName:string;players:number;humans:number;maxPlayers:number;phase:string;fillAi:boolean;publicRoom:boolean};
...
function renderRoomList(rooms:PublicRoomInfo[]){
  ...
  if(!rooms.length){list.innerHTML='<p class="empty-note">현재 공개 대기방이 없습니다.</p>';return;}
  for(const room of rooms){
    ...
    info.innerHTML=`<b>#${escapeText(room.roomCode)}</b><span>${escapeText(room.hostName)} · ${room.players}/${room.maxPlayers} · ${status}${room.fillAi?' · AI':''}</span>`;
...
async function exitRoom(message='',ask=false){
  ...
  $('result').classList.add('hidden');$('lobbyMessages').innerHTML='';$('gameMessages').innerHTML='';$('killfeed').innerHTML='';
