...
export class Drop8Room extends Room<{ state: Drop8State }> {
  ...
  private spawnLoot(){
    ...
    for(const spawn of LOOT_SPAWNS){
      ...
      l.x=pos.x;
      l.y=pos.y;
      l.buildingId=buildingIdAt(l.x,l.y);
  ...
  private dropInventory(p:PlayerState){
    ...
    kinds.forEach((kind,index)=>{
      ...
      loot.id=`loot-${++this.lootSeq}`;loot.kind=kind;loot.x=pos.x;loot.y=pos.y;loot.buildingId=buildingIdAt(loot.x,loot.y);
  ...
  private updateBuildingStates(){
    for(const player of this.state.players.values()){
      ...
      let next='';
      if(current){
        const zone=buildingZoneById(current);
        if(zone){
          const margin=12;
      ...
      if(!next)next=buildingIdAt(player.x,player.y,12);
  ...
  private canSeeTarget(viewer:PlayerState,target:PlayerState){
    return buildingSpacesVisible(viewer,target);
  }
