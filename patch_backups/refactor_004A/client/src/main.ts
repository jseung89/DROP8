...
function render(){
  ...
  $('ammoText').textContent=equipped==='fists'||equipped in MELEE_WEAPONS?'':`${me?.magazine??0} / ${reserveAmmo(me,equipped)}발`;
  $('itemsText').textContent=`붕대 ${me?.bandages??0} · 구급상자 ${me?.medkits??0}`;
  if(me?.healingKind){
    const name=me.healingKind==='medkit'?'구급상자':'붕대';
    $('healText').textContent=`${name} 회복 중 ${Math.round((me.healingProgress??0)*100)}% · 이동/공격 시 취소`;
  }else $('healText').textContent='4 자동 회복';
