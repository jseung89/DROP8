...
export class GameScene extends Phaser.Scene {
  ...
  private updateLocalPrediction(me:any,x:number,y:number,dt:number){
    ...
    const wolf=me.werewolf??{};
    const baseSpeed=wolf.transformed?werewolfSpeed(MOTORCYCLE_MAX_SPEED,Boolean(wolf.sprinting),Boolean(me.insideBuilding),Number(this.net.snapshot?.serverTime??0)<Number(wolf.silverSlowUntil??0)):(me.isSwimming?SWIM_SPEED:PLAYER_SPEED)*multiplier*terrain;
