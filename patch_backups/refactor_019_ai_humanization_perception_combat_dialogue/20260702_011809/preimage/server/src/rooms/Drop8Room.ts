...
export class Drop8Room extends Room<{ state: Drop8State; metadata: Drop8RoomMetadata }> {
  ...
  onDispose(){for(const p of this.state.players.values())if(p.werewolf.transformed)this.endWerewolfCycle(p,'round_end');this.clearCountermeasureState();this.werewolfMoveVectors.clear();void this.presence.del(`drop8:${this.roomId}`);}
  ...
  private fireAdhesiveSprayer(p:PlayerState,magazine:number,now:number){
    ...
    for(const target of this.state.players.values()){
      ...
      if(this.firstObstacleHitT(originX,originY,target.x,target.y,2)!==null)continue;
      target.werewolf.silverSlowUntil=Math.max(target.werewolf.silverSlowUntil,now+2.5);
