...
export class Drop8Room extends Room<{ state: Drop8State }> {
  ...
  private spawnLoot(){
    ...
    const weighted:LootKind[]=[
      'pistol','pistol','smg','rifle','shotgun','small_ammo','small_ammo','rifle_ammo','shells',
      'vest','bandage','bandage','medkit','bat','pan','pipe','knife',
    ];
    for(const spawn of LOOT_SPAWNS){
      const l=new LootState();
      l.id=`loot-${++this.lootSeq}`;
      l.kind=weighted[Math.floor(Math.random()*weighted.length)]!;
      const pos=this.findLootPosition(spawn.x,spawn.y);
      l.x=pos.x;
      l.y=pos.y;
  ...
  private findLootPosition(baseX:number,baseY:number){
    for(let i=0;i<18;i++){
      const radius=i<4?70:70+i*16;
      const angle=i*2.399963;
      const x=clamp(baseX+Math.cos(angle)*radius*(i===0?0:1),40,WORLD_SIZE-40);
      const y=clamp(baseY+Math.sin(angle)*radius*(i===0?0:1),40,WORLD_SIZE-40);
      if(!OBSTACLES.some((r)=>circleHitsRect(x,y,14,r)))return{x,y};
    }
    return{x:clamp(baseX,40,WORLD_SIZE-40),y:clamp(baseY,40,WORLD_SIZE-40)};
  }
  ...
  private chat(c:Client,m:any){
    ...
    if(this.state.phase==='LOBBY'){
      this.broadcast('chat',{channel:'lobby',sender:p.name,text,time:now});
      return;
    }
    if(!p.alive){
      for(const cl of this.clients){
        const q=this.state.players.get(cl.sessionId);
        if(q&&!q.alive)cl.send('chat',{channel:'spectator',sender:p.name,text,time:now});
      }
      return;
    }
    for(const cl of this.clients){
      const q=this.state.players.get(cl.sessionId);
      if(q?.alive&&distance(p.x,p.y,q.x,q.y)<=CHAT_RADIUS)cl.send('chat',{channel:'nearby',sender:p.name,text,time:now,playerId:p.id});
    }
  }
  ...
  private updatePlayers(dt:number){
    for(const p of this.state.players.values()){
      ...
      if(p.phase==='falling'||p.phase==='parachute'){
        ...
        if(p.altitude<=0){p.phase='landed';if(p.ai)p.aiState='SEEK_WEAPON';this.system(`${p.name} 착지`);}
        continue;
      }
  ...
  private tryMove(p:PlayerState,dx:number,dy:number){
    ...
    if(len>18){dx=dx/len*18;dy=dy/len*18;}
    const nx=clamp(p.x+dx,PLAYER_RADIUS,WORLD_SIZE-PLAYER_RADIUS);
    const ny=clamp(p.y+dy,PLAYER_RADIUS,WORLD_SIZE-PLAYER_RADIUS);
    if(!OBSTACLES.some((r)=>circleHitsRect(nx,p.y,PLAYER_RADIUS,r)))p.x=nx;
    if(!OBSTACLES.some((r)=>circleHitsRect(p.x,ny,PLAYER_RADIUS,r)))p.y=ny;
  ...
  private planAi(p:PlayerState,intent:AiIntent){
    ...
    if(target){
      ...
      intent.lastSeenX=target.x;
      intent.lastSeenY=target.y;
      intent.lastSeenUntil=now+2;
      ...
      if(weapon&&weapon.id!=='fists'&&this.getWeaponMagazine(p,weapon.id)<=0){
        if(this.getAmmo(p,weapon.ammoType)>0){
          this.beginReload(p,weapon.id);
          p.aiState='RELOAD';
          intent.state='RELOAD';
          intent.mode='retreat';
          this.setAiDestination(p,intent,p.x-(target.x-p.x),p.y-(target.y-p.y));
          return;
        }
        this.chooseAiWeapon(p,targetDistance,true);
      }
      p.aiState=p.hp<30?'RETREAT':'ENGAGE';
      intent.state=p.aiState;
      const desired=this.desiredRange(p.equipped as EquippedId);
      if(p.hp<30||targetDistance<desired*.48){
        intent.mode='retreat';
        this.setAiDestination(p,intent,p.x-(target.x-p.x),p.y-(target.y-p.y));
      }else if(targetDistance>desired*1.12){
        intent.mode='move';
        this.setAiDestination(p,intent,target.x,target.y);
      }else{
        intent.mode='hold';
        intent.route=[];
        intent.tx=target.x;
    ...
    if(intent.lastSeenUntil>now){
      p.aiState='REPOSITION';
      intent.state='REPOSITION';
      this.setAiDestination(p,intent,intent.lastSeenX,intent.lastSeenY);
      return;
    }

    const noise=this.findRecentNoise(p);
    if(noise){
      p.aiState='INVESTIGATE';
      intent.state='INVESTIGATE';
      this.setAiDestination(p,intent,noise.x,noise.y);
      return;
    }

    this.chooseAiWeapon(p);
    const current=WEAPONS[p.equipped as WeaponId];
    if(current&&current.id!=='fists'&&this.getWeaponMagazine(p,current.id)<=0&&this.getAmmo(p,current.ammoType)>0){
      this.beginReload(p,current.id);
    }

    const loot=this.findBestLoot(p);
    if(loot){
      intent.lootId=loot.id;
      p.aiState=this.aiLootState(loot.kind as LootKind);
      intent.state=p.aiState;
    ...
    p.aiState='PATROL';
    intent.state='PATROL';
    this.setAiDestination(
      p,
      intent,
      this.state.zoneX+(Math.random()-.5)*Math.min(650,this.state.zoneRadius*.45),
      this.state.zoneY+(Math.random()-.5)*Math.min(650,this.state.zoneRadius*.45),
    );
  }
  ...
  private runAi(p:PlayerState,intent:AiIntent,dt:number){
    ...
    intent.stuckFor=progress<1?intent.stuckFor+dt:Math.max(0,intent.stuckFor-dt*2);
    if(intent.stuckFor>.65){
      intent.avoidSign*=-1;
      const detour={
        x:clamp(p.x+Math.cos(angle+intent.avoidSign*Math.PI/2)*220,PLAYER_RADIUS,WORLD_SIZE-PLAYER_RADIUS),
        y:clamp(p.y+Math.sin(angle+intent.avoidSign*Math.PI/2)*220,PLAYER_RADIUS,WORLD_SIZE-PLAYER_RADIUS),
      };
      intent.route.unshift(detour);
      intent.stuckFor=0;
    }
    intent.lastX=p.x;
    intent.lastY=p.y;
  ...
  private findBestLoot(p:PlayerState){
    ...
    for(const l of this.state.loot.values()){
      const d=distance(p.x,p.y,l.x,l.y);
      if(d>950)continue;
      const score=this.aiLootScore(p,l.kind as LootKind)-d*.035;
  ...
  private aiLootScore(p:PlayerState,k:LootKind){
    if(k in WEAPONS&&k!=='fists'){
      ...
      return incoming>held+4?95+(incoming-held):incoming===held?8:0;
    }
  ...
  private setAiDestination(p:PlayerState,intent:AiIntent,x:number,y:number){
    intent.tx=clamp(x,PLAYER_RADIUS,WORLD_SIZE-PLAYER_RADIUS);
    intent.ty=clamp(y,PLAYER_RADIUS,WORLD_SIZE-PLAYER_RADIUS);
    intent.route=this.buildRoute(p.x,p.y,intent.tx,intent.ty);
  }

  private buildRoute(sx:number,sy:number,tx:number,ty:number){
    const source=this.buildingAt(sx,sy);
    const target=this.buildingAt(tx,ty);
    if(source&&target&&source===target)return[{x:tx,y:ty}];
    const route:Point[]=[];
    if(source){const door=this.doorPoints(source);route.push(door.inside,door.outside);}
    if(target){const door=this.doorPoints(target);route.push(door.outside,door.inside);}
  ...
  private dropInventory(p:PlayerState){
    const kinds=[p.primary,p.secondary,p.melee!=='fists'?p.melee:'',p.armor>0?'vest':'',p.bandages?'bandage':'',p.medkits?'medkit':''].filter(Boolean);
    for(const k of kinds){
      const l=new LootState();
      l.id=`loot-${++this.lootSeq}`;
      l.kind=k;
      l.x=clamp(p.x+(Math.random()-.5)*55,20,WORLD_SIZE-20);
      l.y=clamp(p.y+(Math.random()-.5)*55,20,WORLD_SIZE-20);
      this.state.loot.set(l.id,l);
    }
  }
