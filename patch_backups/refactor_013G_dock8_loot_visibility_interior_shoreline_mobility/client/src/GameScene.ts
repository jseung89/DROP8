...
export class GameScene extends Phaser.Scene {
  ...
  private drawStaticMap(){
    ...
    for(const river of this.mapConfig.rivers){
      for(let index=0;index<river.points.length-1;index++){
        ...
        g.lineStyle(width,0x236b91,.92).lineBetween(a.x,a.y,b.x,b.y);
        g.lineStyle(Math.max(18,width*.07),0x58b5d1,.23).lineBetween(a.x,a.y,b.x,b.y);
      }
    }
    ...
    for(const crossing of this.mapConfig.landCrossings){
      const color=crossing.kind==='ford'?0x7a8f75:crossing.kind==='foot_bridge'?0x8b765c:0x555c61;
      g.fillStyle(color,.98).fillRect(crossing.rect.x,crossing.rect.y,crossing.rect.w,crossing.rect.h);
      g.lineStyle(3,0xd5d2c4,.28).strokeRect(crossing.rect.x,crossing.rect.y,crossing.rect.w,crossing.rect.h);
    }
  ...
  private updateBuildingPresentation(viewer:any){
    ...
    this.updateWindowVisionOverlay(viewer);
  }
  ...
  private cutRoomFromOverlay(context:CanvasRenderingContext2D,room:{rect:{x:number;y:number;w:number;h:number}},alpha:number){
    const topLeft=this.worldToScreen(room.rect.x-3,room.rect.y-3);
    const bottomRight=this.worldToScreen(room.rect.x+room.rect.w+3,room.rect.y+room.rect.h+3);
    context.globalAlpha=alpha;
  ...
  private updateWindowVisionOverlay(viewer:any){
    ...
    context.clearRect(0,0,width,height);
    const buildingId=String(viewer?.buildingId??'');
    const roomIndex=Number(viewer?.roomIndex??0);
    if(!buildingId||!roomIndex){canvas.classList.add('hidden');return;}
    const room=this.mapConfig.rooms.find((candidate)=>candidate.index===roomIndex);
    if(!room){canvas.classList.add('hidden');return;}
    canvas.classList.remove('hidden');
    context.globalCompositeOperation='source-over';
    context.globalAlpha=1;
    context.fillStyle='rgba(2,7,11,.74)';
    context.fillRect(0,0,width,height);
    context.globalCompositeOperation='destination-out';
    context.fillStyle='rgba(0,0,0,1)';
    this.cutRoomFromOverlay(context,room,1);
    const adjacentRooms=new Set<number>();
    for(const portal of this.mapConfig.portals){
      if(!portal.allowsVision)continue;
    ...
    for(const adjacentIndex of adjacentRooms){
      ...
      const adjacent=this.mapConfig.rooms.find((candidate)=>candidate.index===adjacentIndex);
      if(adjacent)this.cutRoomFromOverlay(context,adjacent,.68);
    }
    context.globalAlpha=1;
    context.globalCompositeOperation='source-over';
  }
