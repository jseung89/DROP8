...
export class Drop8Room extends Room<{ state: Drop8State; metadata: Drop8RoomMetadata }> {
  ...
  async onCreate(options:JoinOptions) {
    ...
    this.openArenaConfig=this.gameMode==='openArena'?normalizeOpenArenaConfig(options.maxHumans,options.aiCount):undefined;
  ...
  private removePlayer(playerId:string,reason:'left'|'kicked'){
    ...
    this.humanRespawnAt.delete(playerId);this.spawnProtectionUntil.delete(playerId);this.openArenaDeaths.delete(playerId);this.arenaKillStreak.delete(playerId);this.arenaBestStreak.delete(playerId);this.arenaHumanKills.delete(playerId);this.arenaAiKills.delete(playerId);
  ...
  private arenaStatusPayload(){
    ...
    return{lifecycle:this.openArenaLifecycle,humans,maxHumans:this.openArenaConfig?.maxHumans??MAX_PLAYERS,aliveAi,respawningAi:Math.max(0,(this.openArenaConfig?.configuredAiCount??0)-aliveAi),configuredAiCount:this.openArenaConfig?.configuredAiCount??0,totalCombatants:humans+(this.openArenaConfig?.configuredAiCount??0),hostId:this.state.hostId};
  }
  ...
  private beginOpenArena(){
    ...
    this.state.phase='ACTIVE';this.state.winner='';this.state.placements.clear();this.state.worldSize=this.map.width;
    ...
    this.system(`상시 개방 전장이 시작되었습니다. 인간 ${this.openArenaConfig?.maxHumans??0}명 · AI ${this.openArenaConfig?.configuredAiCount??0}명 · 자기장 없음`);
  }
  ...
  private damage(p:PlayerState,amount:number,attackerId:string,reason:string,knockbackOverride=0,hitAngleOverride?:number,kind:DamageKind='other'){
    ...
    if(p.hp<=0){
      ...
      if(this.gameMode==='openArena'){this.dropOpenArenaDeathLoot(p);this.scheduleOpenArenaRespawn(p);this.broadcastOpenArenaScoreboard(true);}
  ...
  private recordOpenArenaKill(attacker:PlayerState,victim:PlayerState){
    ...
    const targetMap=victim.ai?this.arenaAiKills:this.arenaHumanKills;targetMap.set(attacker.id,(targetMap.get(attacker.id)??0)+1);
  }

  private openArenaScoreboardPayload(){
    const rows=[...this.state.players.values()].map((player)=>{const deaths=this.openArenaDeaths.get(player.id)??0;return{id:player.id,name:player.name,ai:player.ai,kills:player.kills,deaths,kd:deaths>0?Number((player.kills/deaths).toFixed(2)):player.kills,streak:this.arenaKillStreak.get(player.id)??0,bestStreak:this.arenaBestStreak.get(player.id)??0,humanKills:this.arenaHumanKills.get(player.id)??0,aiKills:this.arenaAiKills.get(player.id)??0,alive:player.alive};}).sort((a,b)=>b.kills-a.kills||b.streak-a.streak||a.deaths-b.deaths||a.name.localeCompare(b.name,'ko')).slice(0,20);
    return{generatedAt:this.now(),rows,status:this.arenaStatusPayload()};
  }
