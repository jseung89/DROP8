...
export type Snapshot={
  ...
  aliveCount:number;winner:string;werewolfSeason:any;players:any[];tacticalInventories:any[];bullets:any[];rockets:any[];loot:any[];motorcycles:any[];explosions:any[];thrownObjects:any[];smokeFields:any[];fireFields:any[];flameJets:any[];adhesiveJets:any[];stripTraps:any[];supplyDrops:any[];receivedAt:number;
};
...
export class Network {
  ...
  private attach(){
    ...
    for(const type of ['chat','aiDialogue','killfeed','result','error','notice','pong','kicked','pickupResult','slotSwapResult','positionRecovery','vehicleRecovery','characterDeath','audioEvent','arenaStatus','hostChanged','respawnScheduled','respawned','spawnProtection','arenaScoreboard'])this.room.onMessage(type,(p:any)=>{
      ...
      if(type==='arenaScoreboard'){this.arenaScoreboard=Array.isArray(p?.rows)?p.rows:[];this.arenaStatus=p?.status??this.arenaStatus;this.listeners.forEach(fn=>fn());}
  ...
  private toSnapshot(s:any,receivedAt:number):Snapshot{
    ...
    const snapshot:Snapshot={
      ...
      aliveCount:s.aliveCount,winner:s.winner,werewolfSeason,
  ...
  async leave(consented=true){
    ...
    this.respawnAt=0;this.spawnProtectionUntil=0;this.arenaStatus=null;this.arenaScoreboard=[];
