...
function renderInventory(me:any){
  ...
  const details=[
    ...
    ['근접',weaponName(melee)],['점착 충전량',String(me?.adhesiveCharge??0)],['스트립 트랩',String(me?.stripTrapCount??0)],['권총탄',String(me?.pistolAmmo??0)],['일반 총알',String(me?.standardAmmo??0)],['샷건탄',String(me?.shotgunAmmo??0)],['로켓탄',String(me?.rocketAmmo??0)],['연료',String(me?.fuelAmmo??0)],
...
function render(){
  ...
  if(me?.isDriving&&motorcycle){
    ...
    const slowText=slowRemaining>0?(slowKind==='strip_trap'?'타이어 손상':slowKind==='mixed'?'복합 감속':'점착 감속')+` · ${slowRemaining.toFixed(1)}초 · 최고속도 ${Math.round(Number(motorcycle.slowSpeedMultiplier??1)*100)}%`:'';
