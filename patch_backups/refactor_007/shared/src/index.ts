...
export interface MapConfig {
  ...
  regions:Region[]; buildings:Building[]; buildingVisibilityZones:BuildingVisibilityZone[]; decorations:Decoration[]; worldProps:WorldProp[]; obstacles:Rect[]; propObstacles:Rect[]; collisionObstacles:Rect[]; bulletObstacles:Rect[]; bushes:Bush[]; lootSpawns:Array<{x:number;y:number;region:string;regionId:RegionId}>; emergencySpawnPoints:Array<{x:number;y:number}>;
}

export const AMMO_DISPLAY_NAMES:Record<Exclude<AmmoType,'none'>,string>={shotgun_ammo:'샷건탄',standard_ammo:'일반 총알',pistol_ammo:'권총탄'};
export const RENDER_DEPTH={GROUND:0,FLOOR_DECORATION:5,GROUND_ITEM:10,WORLD_PROP:20,PLAYER:30,PLAYER_OVERLAY:35,BUILDING_ROOF:50,PLANE_SHADOW:80,TRANSPORT_PLANE:100,PLANE_EFFECT:105,WORLD_EFFECT:120,HUD:1000,MODAL:2000} as const;
