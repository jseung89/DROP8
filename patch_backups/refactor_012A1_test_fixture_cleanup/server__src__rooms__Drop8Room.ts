...
export class Drop8Room extends Room<{ state: Drop8State }> {
  ...
  private applyThrowableLoot(p:PlayerState,type:ThrowableType,source?:LootState):{success:boolean}{
    ...
    if(p.throwableType&&p.throwableCount>0&&isThrowableType(p.throwableType)){
      const point=this.findSeparatedLootPosition(p.x- Math.cos(p.angle)*42,p.y-Math.sin(p.angle)*42,p.throwableType as LootKind);
      if(!point)return{success:false};
      const drop=new LootState();drop.id=`loot-${++this.lootSeq}`;drop.kind=p.throwableType;drop.stackCount=p.throwableCount;drop.x=point.x;drop.y=point.y;drop.buildingId=p.buildingId;drop.pickupLockedForPlayerId=p.id;drop.pickupLockedUntil=this.now()+.85;
  ...
  private updateThrowables(dt:number){
    ...
    for(const object of [...this.state.thrownObjects.values()]){
      ...
      if(type==='incendiaryGrenade'&&(next.collision!=='none'||next.phase==='resting')){this.spawnFireField(object);this.state.thrownObjects.delete(object.id);continue;}
      if(type==='smokeGrenade'&&next.phase==='resting'&&object.detonateAt<=0)object.detonateAt=now+SMOKE_TIMING.deployDelayMs/1000;
      if(type==='smokeGrenade'&&object.detonateAt>0&&now>=object.detonateAt){this.spawnSmokeField(object);this.state.thrownObjects.delete(object.id);continue;}
  ...
  private dropInventory(p:PlayerState){
    ...
    if(p.throwableType&&p.throwableCount>0&&isThrowableType(p.throwableType))kinds.push(p.throwableType);
    ...
    kinds.forEach((kind,index)=>{
      ...
      loot.id=`loot-${++this.lootSeq}`;loot.kind=kind;loot.x=pos.x;loot.y=pos.y;loot.buildingId=buildingIdAt(loot.x,loot.y,0,this.map.buildingVisibilityZones);if(isThrowableType(kind))loot.stackCount=p.throwableCount;
