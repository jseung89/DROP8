...
import {
  BUILDINGS, LOOT_COLORS, LOOT_LABELS, MELEE_WEAPONS, OBSTACLES, REGIONS, WORLD_SIZE,
  type EquippedId, type LootKind
} from '@drop8/shared';
...
export class GameScene extends Phaser.Scene {
  ...
  update(time:number){
    ...
    if(me){
      ...
      const shown=this.getDisplayPlayer(target.id,target.x,target.y,me.id===target.id ? .42 : .24);
      ...
      if(time-this.lastInput>=50){
        this.net.send('input',{x,y,angle:Math.atan2(pointer.worldY-me.y,pointer.worldX-me.x),seq:++this.seq});
        this.lastInput=time;
      }
  ...
  private drawDynamic(time:number){
    ...
    g.lineStyle(7,0x4db5ff,.78).strokeCircle(s.zoneX,s.zoneY,s.zoneRadius);
    g.lineStyle(3,0xffffff,.22).strokeCircle(s.zoneX,s.zoneY,s.nextZoneRadius);

    for(const l of s.loot){
      if(!visible(l.x,l.y,70))continue;
      this.drawLootIcon(g,l.kind as LootKind,l.x,l.y,1);
    }
    ...
    for(const p of s.players){
      ...
      activeIds.add(p.id);
      const pos=this.getDisplayPlayer(p.id,p.x,p.y,p.id===this.net.sessionId ? .45 : .22);
      this.drawPlayer(g,p,pos.x,pos.y,time);
    }
  ...
  private drawPlayer(g:Phaser.GameObjects.Graphics,p:any,x:number,y:number,time:number){
    ...
    g.fillStyle(color).fillCircle(x,y,20*scale);
